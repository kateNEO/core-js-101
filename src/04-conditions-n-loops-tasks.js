...
function isCreditCardNumber(ccn) {
  const CCN = Array.from(String(ccn));
  let sum = 0;
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(Array.from(String(num)).map(Number).reduce((sum, digit) => sum + digit, 0));
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (openBr.includes(str[i])) {
      stack.push(str[i]);
    } else if (openBr.indexOf(stack[stack.length - 1]) === closeBr.indexOf(str[i])) {
      stack.pop();
    } else return false;
  }
